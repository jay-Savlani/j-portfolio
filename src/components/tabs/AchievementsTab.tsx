import { List } from "../utilityComponents/List";
import { achievementsData } from "./achievementsTab/achievementsData";

export const AchievementsTab = () => {
  return (
    <List
      items={achievementsData}
      classes="animation-fade-in pt-3"
      ListItem={({ item }) => {
        return (
          <li key={item.achievement}>
            <div className="flex flex-col grow px-10 gap-6 justify-center">
              <div className="flex flex-row gap-10 items-center">
                <img className="h-60 w-60" src={item.imgSrc} />

                <h2 className="text-[24px] text-[18px] text-center text-wrap mx-auto">
                  {item.achievement}
                </h2>

                <button className="bg-gradient-to-r from-violet-500 to-fuchsia-500 p-5 rounded hover:opacity-70 ms-auto">
                  <a
                    className="no-underline text-white hover:text-slate-100"
                    href={item.certificateLink}
                    target="_blank"
                  >
                    View Certificate
                  </a>
                </button>
              </div>
            </div>
          </li>
        );
      }}
    />
  );
};
